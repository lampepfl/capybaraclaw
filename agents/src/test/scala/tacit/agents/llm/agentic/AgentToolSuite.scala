...
object RollDice extends AgentTool[AgentState]:
  ...
  override def handle(arg: DiceArgs, state: AgentState): String =
    s"Rolled ${arg.count} dice"
...
object WebSearch extends AgentTool[AgentState]:
  ...
  override def handle(arg: SearchArgs, state: AgentState): String =
    s"Searching for ${arg.query}"
...
object TagItems extends AgentTool[AgentState]:
  ...
  override def handle(arg: TagArgs, state: AgentState): String =
    s"Tagged ${arg.names.size} items"
...
class AgentToolSuite extends munit.FunSuite:
  ...
  test("toolSchema: list fields have array type with items"):
    ...
    assertEquals(
      schema.parameters.properties("names").items.get.`type`,
      "string"
    )
  ...
  test("parseArgs: tool with optional field present"):
    val result =
      WebSearch.parseArgs("""{"query": "scala 3", "maxResults": 10}""")
  ...
  test("parseArgs: tool with list field"):
    val result =
      TagItems.parseArgs("""{"names": ["a", "b"], "verbose": true}""")
  ...
  test("parseArgs: wrong type"):
    ...
    assert(result.isLeft)
    assert(
      result.swap.getOrElse(fail("expected Left")).message.contains("count")
    )

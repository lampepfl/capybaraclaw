...
object ToolArgParsingError:
  ...
  def typeMismatch(
    ...
    ToolArgParsingError(
      s"Field '$fieldName': expected $expected, got ${got.render()}"
    )
...
object IsFieldType:
    ...
    new IsFieldType[T]:
      ...
      def parse(v: ujson.Value): Result[T, ToolArgParsingError] =
        try Right(f(v))
          ...
          case e: Exception =>
            Left(
              ToolArgParsingError(s"Expected $tpe, got: ${v.render()}", Some(e))
            )
  ...
  given [T](using inner: IsFieldType[T]): IsFieldType[List[T]] with
    ...
    def items = Some(
      ToolSchema.Property(`type` = inner.schemaType, items = inner.items)
    )
    def parse(v: ujson.Value): Result[List[T], ToolArgParsingError] =
      try
        ...
        case e: Exception =>
          Left(
            ToolArgParsingError(s"Expected array, got: ${v.render()}", Some(e))
          )
...
object IsToolArg:
  private[utils] class DerivedIsToolArg[T](
    ...
    def schema: ToolSchema.Parameters =
      val properties = labels.zip(fieldTypes).zip(descriptions).map {
        case ((name, ft), desc) =>
          name -> ToolSchema.Property(
            `type` = ft.schemaType,
            description = desc,
            items = ft.items
          )
      ...
      val obj =
        try json.obj
          ...
          case e: Exception =>
            return Left(
              ToolArgParsingError(
                s"Expected JSON object, got: ${json.render()}"
              )
            )
      Result:
        val values = labels
          .zip(parsers)
          .map: (name, parser) =>
            if obj.contains(name) then parser(name, obj(name)).ok
            else parser(name, ujson.Null).ok
  ...
  inline def getFieldDescriptions[T]: List[String] = ${
    getFieldDescriptionsImpl[T]
  }

  private def getFieldDescriptionsImpl[T: Type](using
      Quotes
  ): Expr[List[String]] =
    import quotes.reflect.*
    val sym = TypeRepr.of[T].typeSymbol
    val ctorParams =
      sym.primaryConstructor.paramSymss.flatten.filterNot(_.isType)
  ...
  private[utils] def parseParamTags(raw: Option[String]): Map[String, String] =
    raw match
      ...
      case Some(s) =>
        ...
        val scaladocTags =
          """@(?:param|return|returns|throws|tparam|see|note|example|constructor|deprecated|since|version|todo|inheritdoc|group)\b"""
        val paramPattern =
          s"""@param\\s+(\\w+)\\s+((?:(?!$scaladocTags).)+)""".r
        val cleaned = s
          .stripPrefix("/**")
          .stripSuffix("*/")
          .linesIterator
  ...
  private inline def getFieldLabels[T <: Tuple]: List[String] =
    inline erasedValue[T] match
      ...
      case _: (t *: ts)  =>
        constValue[t].asInstanceOf[String] :: getFieldLabels[ts]
  ...
  private inline def getFieldParsers[T <: Tuple]
      : List[(String, ujson.Value) => Result[Any, ToolArgParsingError]] =
    inline erasedValue[T] match
      case _: EmptyTuple => Nil
      case _: (t *: ts)  =>
        val ft = summonInline[IsFieldType[t]]
        val parser: (String, ujson.Value) => Result[Any, ToolArgParsingError] =
          (name, v) =>
            ft.parse(v) match
              case Right(value)                 => Right(value)
              case Left(err) if v == ujson.Null =>
                Left(ToolArgParsingError.missingField(name))
              case Left(_) =>
                Left(ToolArgParsingError.typeMismatch(name, ft.schemaType, v))
        parser :: getFieldParsers[ts]

  private inline def getRequiredFields[Types <: Tuple, Labels <: Tuple]
      : List[String] =

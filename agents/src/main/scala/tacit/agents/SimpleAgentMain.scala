...
@main def simpleAgentRepl(): Unit =
  given Endpoint = OpenRouterEndpoint.createFromEnv()
  ...
  val agent = new Agent:
    ...
    def getInitState = new AgentState:
      val llmConfig = LLMConfig(
        ...
        maxTokens = Some(4096),
        thinking = Some(ThinkingMode.Effort(EffortLevel.XHigh))
      )
  ...
  agent
    ...
    .handle[CalcArgs]("calculate", "Evaluate a simple arithmetic expression"):
      (args, _) =>
        try
          val result = javax.script
            .ScriptEngineManager()
            .getEngineByName("js")
    ...
    .handle[CoinFlipArgs](
      "flipCoins",
      "Flip a number of coins and return heads/tails results"
    ): (args, _) =>
      val results = (1 to args.count)
        .map(_ => if scala.util.Random.nextBoolean() then "heads" else "tails")
      s"""{"results": [${results.map(r => s""""$r"""").mkString(", ")}]}"""
    .handle[CountdownArgs](
      "countdown",
      "Count down from a number, returning each step"
    ): (args, _) =>
      val steps = (args.from to 0 by -1).toList
      s"""{"steps": [${steps.mkString(", ")}], "message": "Liftoff!"}"""

  println("Chat with Simple Agent (type 'quit' to exit)")
  println()
      ...
      Async.blocking:
        ...
        while reading do
          ch.read() match
            case Right(
                  Right(
                    AgentStreamEvent.Stream(StreamEvent.ThinkingDelta(text))
                  )
                ) =>
              if !inThinking then
                println("<thinking>")
              ...
              System.out.flush()
            case Right(
                  Right(AgentStreamEvent.Stream(StreamEvent.Delta(text)))
                ) =>
              if inThinking then
                print("\n</thinking>\n\n")
                inThinking = false
              ...
              System.out.flush()
            case Right(
                  Right(AgentStreamEvent.ToolResult(_, toolName, result))
                ) =>
